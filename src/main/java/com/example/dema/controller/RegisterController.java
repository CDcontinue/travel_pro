...
public class RegisterController {
    ...
    @ResponseBody
    public Object findUsernameIsUsed(@RequestParam("account") String account){
        HashMap<String,String> selectResult = new HashMap<String, String>();
        int res = -1;
        try {
            res = registerService.findUsernameIsUsed(account);
        } catch (Exception e) {
            e.printStackTrace();
        }finally {
            if (res > 0){
    ...
    @ResponseBody
    public Object findPhoneNumIsUsed(@RequestParam("phone") String phone){
        HashMap<String,String> selectResult = new HashMap<String, String>();
        int res = -1;
        try {
            res = registerService.findPhoneNumIsUsed(phone);
        } catch (Exception e) {
            e.printStackTrace();
        }finally {
            if (res > 0){
